...
public class ServicesGeneratorCodePlug : IMergeBlockGeneratorCodePlug
{
    ...
    private async Task GeneratorIServicesCodeAsync(GeneratorCodeContext context, DomainModel domain)
    {
        ...
        codeContent.AppendLine($"namespace {context.ProjectName}.{context.ModuleName}.Abstractions.Services");
        codeContent.AppendLine($"{{");
        ...
        codeContent.AppendLine($"    }}");
        codeContent.AppendLine($"}}");
        context.SaveAs(codeContent, context.ModuleAbstractionsMGCPath, "Services", $"I{domain.Name}Service.cs");
    }
    ...
    private async Task GeneratorServiceImplsCodeAsync(GeneratorCodeContext context, DomainModel domain, List<DomainModel> domains)
    {
        if (domain.HasAttribute<NotServiceAttribute>()) return;
        ...
        codeContent.AppendLine($"using {context.ProjectName}.{context.ModuleName}.Abstractions.Services.Models.{domain.Name};");
        codeContent.AppendLine($"");
        codeContent.AppendLine($"namespace {context.ProjectName}.{context.ModuleName}.Application.Services");
        codeContent.AppendLine($"{{");
        codeContent.AppendLine($"    /// <summary>");
        codeContent.AppendLine($"    /// {domain.Annotation}服务");
        codeContent.AppendLine($"    /// </summary>");
        if (domain.HasAttribute<EmptyServiceAttribute>())
        {
            ...
            {
                codeContent.AppendLine($"    public partial class {domain.Name}ServiceImpl : BaseServiceImpl<I{context.ModuleName}UnitOfWork>, I{domain.Name}Service, IScopedDependency<I{domain.Name}Service>");
            }
            else
            {
                if (targetDomain != domain)
                {
                    codeContent.AppendLine($"    public partial class {domain.Name}ServiceImpl : BaseServiceImpl<I{domain.Name}Repository, I{targetDomain.Name}Repository, {domain.Name}, {targetDomain.Name}, I{context.ModuleName}UnitOfWork>, I{domain.Name}Service, IScopedDependency<I{domain.Name}Service>");
                }
                else
                {
                    codeContent.AppendLine($"    public partial class {domain.Name}ServiceImpl : BaseServiceImpl<I{domain.Name}Repository, {domain.Name}, I{context.ModuleName}UnitOfWork>, I{domain.Name}Service, IScopedDependency<I{domain.Name}Service>");
                }
            }
        }
        else
        {
            if (targetDomain != domain)
            {
                codeContent.AppendLine($"    public partial class {domain.Name}ServiceImpl : BaseServiceImpl<Add{domain.Name}Model, Edit{domain.Name}Model, Query{domain.Name}Model, {domain.Name}DTO, {domain.Name}ListDTO, I{domain.Name}Repository, I{targetDomain.Name}Repository, {domain.Name}, {targetDomain.Name}, I{context.ModuleName}UnitOfWork>, I{domain.Name}Service, IScopedDependency<I{domain.Name}Service>");
            }
            else
            {
                codeContent.AppendLine($"    public partial class {domain.Name}ServiceImpl : BaseServiceImpl<Add{domain.Name}Model, Edit{domain.Name}Model, Query{domain.Name}Model, {domain.Name}DTO, {domain.Name}ListDTO, I{domain.Name}Repository, {domain.Name}, I{context.ModuleName}UnitOfWork>, I{domain.Name}Service, IScopedDependency<I{domain.Name}Service>");
            }
        }
        codeContent.AppendLine($"    {{");
        ...
        if (domain.IsTreeDomain && !domain.HasAttribute<EmptyTreeAttribute>())
        {
            ...
            codeContent.AppendLine($"            }}");
            codeContent.AppendLine($"            #endregion");
            codeContent.AppendLine($"            Dictionary<string, object> contextData = [];");
            codeContent.AppendLine($"            OnToTreeBefore(allInfo, queryModel, contextData);");
            ...
            codeContent.AppendLine($"                OnConvertToTreeDTO(dto, domain, queryModel, contextData);");
            codeContent.AppendLine($"            }});");
            codeContent.AppendLine($"            OnToTreeAfter(result, queryModel, contextData);");
            ...
            codeContent.AppendLine($"        /// <param name=\"queryModel\"></param>");
            codeContent.AppendLine($"        partial void OnToTreeBefore(List<{domain.Name}> allInfo, Query{domain.Name}TreeListModel queryModel, Dictionary<string, object> contextData);");
            codeContent.AppendLine($"        /// <summary>");
            codeContent.AppendLine($"        /// 转换树之后");
            codeContent.AppendLine($"        /// </summary>");
            codeContent.AppendLine($"        /// <param name=\"dtos\"></param>");
            codeContent.AppendLine($"        /// <param name=\"queryModel\"></param>");
            codeContent.AppendLine($"        partial void OnToTreeAfter(List<{domain.Name}TreeListDTO> dtos, Query{domain.Name}TreeListModel queryModel, Dictionary<string, object> contextData);");
            codeContent.AppendLine($"        /// <summary>");
            ...
            codeContent.AppendLine($"        /// <param name=\"queryModel\"></param>");
            codeContent.AppendLine($"        partial void OnConvertToTreeDTO({domain.Name}TreeListDTO dto, {domain.Name} domain, Query{domain.Name}TreeListModel queryModel, Dictionary<string, object> contextData);");
        }
        codeContent.AppendLine($"    }}");
        codeContent.AppendLine($"}}");
        context.SaveAs(codeContent, context.ModuleApplicationMGCPath, "Services", $"{domain.Name}ServiceImpl.cs");
    }
}
