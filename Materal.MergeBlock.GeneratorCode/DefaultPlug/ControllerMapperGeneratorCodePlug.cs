...
public class ControllerMapperGeneratorCodePlug : IMergeBlockGeneratorCodePlug
{
    ...
    private async Task GeneratorIControllerMapperCodeAsync(GeneratorCodeContext context, IServiceModel service)
    {
        ...
        codeContent.AppendLine($"    }}");
        codeContent.AppendLine($"}}");
        context.SaveAs(codeContent, context.ModuleAbstractionsMGCPath, "Controllers", $"I{service.DomainName}Controller.Mapper.cs");
    }
    ...
    private async Task GeneratorControllerMapperCodeAsync(GeneratorCodeContext context, IServiceModel service)
    {
        ...
        foreach (MethodModel method in service.Methods)
        {
            ...
            for (int i = 0; i < method.Arguments.Count; i++)
            {
                ...
                {
                    mapperCodes.Add($"            {methodArgument.PredefinedType} {methodArgument.Name} = Mapper.Map<{methodArgument.PredefinedType}>({methodArgument.RequestName}) ?? throw new {context.ProjectName}Exception(\"映射失败\");");
        ...
        codeContent.AppendLine($"    }}");
        codeContent.AppendLine($"}}");
        context.SaveAs(codeContent, context.ModuleApplicationMGCPath, "Controllers", $"{service.DomainName}Controller.Mapper.cs");
    }
