...
**必须在以下变更后运行代码生成器**，否则会导致编译错误或生成不完整的代码：

| 变更类型 | 说明 | 示例 |
|----------|------|------|
| 实体变更 | 新增、删除、修改实体类 | 添加 `Admin` 实体 |
| 枚举变更 | 新增、删除、修改枚举类 | 添加 `XXXStatus` 枚举 |
| 服务接口变更 | 新增、删除、修改服务接口 | 在 `IAuthService` 中添加新方法 |

**开发流程**：
1. 设计并创建实体、服务接口、请求模型
2. 运行 `MMB GeneratorCode`
3. 验证生成结果
4. 实现服务方法
5. 运行项目

**重要**：每次修改服务接口（如添加 `[MapperController]` 标记的方法）后，必须重新运行代码生成器，否则：
- 新的控制器接口方法不会被生成
- 旧的代码可能与新接口不匹配
